...
const wattsonic_1 = require("../wattsonic");
const RETRY_INTERVAL = 28 * 1000;
class MyWattsonicBatteryDevice extends wattsonic_1.Wattsonic {
    /**
    ...
    async pollInvertor() {
        ...
        socket.on('connect', async () => {
            console.log('Connected ...');
            console.log(modbusOptions);
            const checkRegisterRes = await (0, response_1.checkHoldingRegisterWattsonic)(this.holdingRegistersBattery, client);
            console.log('disconnect');
            client.socket.end();
            socket.end();
