...
exports.Solaredge = void 0;
const homey_1 = __importDefault(require("homey"));
function sleep() {
    return new Promise((resolve) => setTimeout(resolve, 1000));
}
class Solaredge extends homey_1.default.Device {
    constructor() {
        super(...arguments);
        this.registers = {
    ...
    validResultRecord(entry) {
        if (entry && entry.value && entry.value != 'xxx' && entry.scale) {
            return true;
        }
        return false;
    }
    validResultScaleRecord(entry) {
        if (entry && entry.scale) {
            return true;
        }
        return false;
    }
    processResult(result, maxpeakpower) {
        if (result) {
            // result
            for (let k in result) {
            ...
            if (this.validResultRecord(result['powerreduce']) && this.hasCapability('powerreduce')) {
                this.addCapability('powerreduce');
